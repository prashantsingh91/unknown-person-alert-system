    ...
    return (
      <div className="bg-white rounded-lg shadow-md overflow-hidden border border-gray-200">
        ...
        <div className="p-4 max-h-80 overflow-y-auto">
          {persons.length === 0 ? (
            <p className="text-gray-600 text-center py-4">No detections yet</p>
          ) : (
              ...
              {persons.slice(0, 10).map((person, index) => (
                ...
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-gray-900">{person.person_name}</p>
                      <p className="text-xs text-gray-600">ID: {person.person_id}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-gray-600">{formatTimestamp(person.timestamp)}</p>
                    <p className="text-xs text-green-600">{(person.similarity * 100).toFixed(0)}%</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="p-6">
        {persons.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-600 text-lg">No known persons detected yet</p>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <p className="text-lg font-bold text-gray-900">{person.person_name}</p>
                      <p className="text-sm text-gray-600">Person ID: {person.person_id}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-700">{formatTimestamp(person.timestamp)}</p>
                    <div className="mt-2">
